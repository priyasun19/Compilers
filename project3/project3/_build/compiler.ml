...
let rec emit_exp (e:exp) (stream : insn list) : insn list = 
 begin
	...
	| Binop (op,e1,e2) -> 		
		...
		resstream
		(*List.append [Push (ecx);Add (Imm 8l,esp)] resstream*)
	| Unop (op,e1) -> begin
...
(*
 let body = [Push (ebp);Mov (esp,ebp);Mov ((stack_offset 8l), ecx);Push (ecx);Pop (eax);Pop (ebp);Ret] in
 	[Cunit.Code ({label = mk_lbl_named "program";insns = body;global = true})]*)
